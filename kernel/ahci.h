...
void nrobe_port(HBA_MEM *abar)
{
	...
	while (i<32)
	{
		if (pi & 1)
		{
			...
			if (dt == AHCI_DEV_SATA)
			{
				kprint(0x2, "SATA drive found at port %d\n");
				//int test = abar->pi;
				while(1);
			}
			...
			{
				kprint(0x2, "SATAPI drive found at port %d\n");
			}
			else if (dt == AHCI_DEV_SEMB)
			{
				kprint(0x2, "SEMB drive found at port %d\n");
			}
			else if (dt == AHCI_DEV_PM)
			{
				kprint(0x2, "PM drive found at port %d\n");
			}
			else
			{
				//kprint(0x2, "No drive found at port %d\n");
			}
		}
